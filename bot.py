...
async def intelligence_scanner():
    ...
    try:
        ...
        for coin in coins:
            ...
            bbw_1h = float(coin.get('bbw_1h') or 0) 
            bbw_prev_1h = float(coin.get('bbw_prev_1h') or 0) 
            expansion_ratio_1h = (bbw_1h / bbw_prev_1h) if bbw_prev_1h > 0 else 1.0 
             
            bbw_2h = float(coin.get('bbw_2h') or 0) 
            bbw_prev_2h = float(coin.get('bbw_prev_2h') or 0) 
            expansion_ratio_2h = (bbw_2h / bbw_prev_2h) if bbw_prev_2h > 0 else 1.0 
            
            bbw_4h = float(coin.get('bbw_4h') or 0) 
            bbw_prev_4h = float(coin.get('bbw_prev_4h') or 0) 
            expansion_ratio_4h = (bbw_4h / bbw_prev_4h) if bbw_prev_4h > 0 else 1.0 
            ...
            # 1. متغيرات البولنجر والكيلتنر 1h لكسر الانضغاط الماكرو
            bb_lower_1h = float(coin.get('bb_lower_1h') or 0)
            kc_upper_1h = float(coin.get('kc_upper_1h') or 0)
            kc_lower_1h = float(coin.get('kc_lower_1h') or 0)
            
            # 2. متغيرات قوة الاتجاه (ADX) للفريمات الكبيرة
            ...
            # ==========================================
            # 🌟 استخراج متغيرات استراتيجية "فخ التلاعب بالسيولة"
            # ==========================================
            rsi_1h = float(coin.get('rsi_1h') or 50)
            obv_slope_2h = float(coin.get('obv_slope_2h') or 0)
            obv_slope_4h = float(coin.get('obv_slope_4h') or 0)
            taker_buy_ratio_1h = float(coin.get('taker_buy_ratio_1h') or 1.0)
            funding_rate = float(coin.get('funding_rate') or 0) # 👈 تم إضافة سحب معدل التمويل
            
            # ==========================================
            # 🐋 [ الاستراتيجية 1: فخ التلاعب بالسيولة (Short Squeeze) ]
            # ==========================================
            # 1. حالة الذعر والقاع: السعر لمس البولنجر السفلي أو قريب جداً من EMA 20
            is_panic_zone = (price <= bb_lower_1h * 1.02) or (abs(price - ema20_1h) / ema20_1h <= 0.015)
            
            # 2. التشبع البيعي (سمحنا للـ RSI بالوصول لـ 40 للسماح بشمعة الاختراق بالظهور)
            is_oversold = (rsi_1h < 40)
            
            # 3. الانحراف الخفي: السعر هبط لكن الحيتان تشتري بضراوة (OBV صاعد)
            is_hidden_divergence = (obv_slope_2h > 0) and (obv_slope_4h > 0)
            
            # 4. تدفق سيولة الماركت الشرائية (Taker Buy Ratio)
            is_whale_absorbing = (taker_buy_ratio_1h >= 1.05)
            
            # 5. التناقض الصارخ للتمويل: القطيع يراهن على الهبوط بكثافة (تمويل سلبي)
            is_funding_negative = (funding_rate < 0) # 👈 القطيع يدفع للحيتان!
            
            # 6. تأكيد الانعكاس: السعر يخترق أو يتداول بثبات فوق EMA 20 على فريم الساعة
            is_breaking_ema20 = (price >= ema20_1h)

            # التنفيذ الحاسم:
            if is_panic_zone and is_oversold and is_hidden_divergence and is_whale_absorbing and is_funding_negative and is_breaking_ema20:
                score += 360
                signal_type = "LONG"
                reasons.append("🐋 فخ التلاعب بالسيولة: الحيتان يمتصون العرض بينما القطيع يراهن على الهبوط (تمويل سلبي).")
                reasons.append(f"⚡ تأكيد الانعكاس: شراء ماركت عنيف (Taker Ratio: {taker_buy_ratio_1h:.2f}) واختراق خط EMA 20.")
                
            # ==========================================
            # 🌟 استخراج متغيرات استراتيجية "قدر الضغط العرضي"
            # ==========================================
            ...
            whale_net_flow = float(coin.get('whale_net_flow_volume') or 0)
            
            # ==========================================
            # 🌋 [ الاستراتيجية 2: قدر الضغط العرضي والانفجار ] (التقييم: 300)
            # ==========================================
            
            # 1. الاتجاه العام: مسار "عرضي" على فريم الساعة واليومي
            is_sideways_macro = (trend_1h == 'عرضي') and (trend_1d == 'عرضي')
            
            # 2. السعر العام: البيئة آمنة للتداول (فوق متوسط 200 اليومي)
            # إذا لم يكن ema200_1d متوفراً لسبب ما، نعتبره True لكي لا تتعطل الإشارة
            is_above_ema200 = (price > ema200_1d) if ema200_1d > 0 else True
            
            # 3. كذبة الانضغاط: يجب ألا يكون الباند منضغطاً بشكل كلاسيكي
            is_not_squeezed = not was_squeezed_1h
            
            # 4. الغليان الخفي: زخم مرعب مخفي داخل المسار العرضي
            is_boiling_adx = (adx_1h > 45)
            
            # 5. تجميع الحيتان: يبنون مراكزهم صعوداً دون انتظار القيعان (RSI بين 60 و 75)
            is_whale_accumulating = (60 <= rsi_1h <= 75) and (60 <= rsi_4h <= 75)
            
            # 6. تأكيد السيولة: ميل إيجابي للأموال وصافي تدفق يفوق المليون ونصف دولار
            has_liquidity_flow = (obv_slope_1h > 0) and (whale_net_flow > 1500000)
            
            # 7. التنفيذ (الزناد): الماكد يعطي الإشارة الخضراء للانطلاق
            is_macd_trigger = (macd_hist_1h > 0)

            # 🚀 إطلاق الاستراتيجية
            if is_sideways_macro and is_above_ema200 and is_not_squeezed and is_boiling_adx and is_whale_accumulating and has_liquidity_flow and is_macd_trigger:
                score += 400
                signal_type = "LONG"
                reasons.append("🌋 قدر الضغط العرضي: مسار عرضي يغلي من الداخل بزخم (ADX > 45) وتجميع حيتان شرس.")
                reasons.append(f"💰 سيولة هائلة: صافي دخول الحيتان يتجاوز {whale_net_flow / 1000000:.2f} مليون دولار (انفجار وشيك).")
                reasons.append("⚡ الماكد (MACD) يعطي تقاطع الانطلاق المباشر لركوب الموجة.")
                
            # ==========================================
            # 🛡️ [ فلاتر الحماية الصارمة - تشريح مسرح الجريمة ]
            # ==========================================
            # 1. فخ التقاطع الذهبي الوهمي: يجب ألا يكون السعر تحت EMA 200 لفريم 4 ساعات لكي لا نُصطدم بسقف حديدي
            no_fake_cross_trap = (price > ema200_4h) if ema200_4h > 0 else True
            
            # 2. فخ معدل التمويل (Funding Trap): القطيع يشتري (تمويل إيجابي) لكن السيولة (OBV) تهبط!
            # إذا كان الفندنج إيجابياً عالياً والـ OBV ينحدر، نرفض الصفقة فوراً.
            no_funding_trap = not ((funding_rate > 0.005) and (obv_slope_1h < 0))
            
            # 3. قاعدة الجاذبية للمتوسطات: نمنع الشراء إذا طار السعر وابتعد عن EMA 50 (فريم 4H) بأكثر من 18% (حالة تشبع وإنهاك)
            distance_from_ema50_4h = ((price - ema50_4h) / ema50_4h) if ema50_4h > 0 else 0
            is_within_gravity = (distance_from_ema50_4h <= 0.18)
            
            # ==========================================
            # 👑 [ الاستراتيجية 3: التوافق المثالي للاتجاه (The Holy Grail) ]
            # ==========================================
            # 1. إطار اليومي: السعر يتداول بوضوح فوق EMA 200 (نحن في سوق صاعد)
            is_macro_bullish = (price > ema200_1d) if ema200_1d > 0 else True
            
            # 2. إطار 4 ساعات: الماكد إيجابي (زخم استراتيجي داعم)
            is_4h_momentum_up = (macd_hist_4h > 0)
            
            # 3. إطار الساعة: ترتيب المتوسطات العسكري (20 > 50 > 100)
            is_perfect_alignment = (ema20_1h > ema50_1h) and (ema50_1h > ema100_1h)
            
            # 4. الزخم والسيولة على فريم الساعة
            is_trend_confirmed = (macd_hist_1h > 0) and (obv_slope_1h > 0) and (adx_1h > 25)

            # 🚀 إطلاق الاستراتيجية الحصينة
            if (is_macro_bullish and is_4h_momentum_up and is_perfect_alignment and is_trend_confirmed 
                and no_fake_cross_trap and no_funding_trap and is_within_gravity):
                
                score += 380  # تقييم عالي جداً لصرامة الشروط
                signal_type = "LONG"
                reasons.append("👑 التوافق المثالي: ترتيب مثالي للمتوسطات (1H) وزخم إيجابي (MACD) مدعوم باتجاه يومي صاعد.")
                reasons.append(f"⚡ قوة الاتجاه: سيولة متدفقة (OBV صاعد) وقوة ترند خالية من الاختراقات الوهمية (ADX: {adx_1h:.1f}).")
                reasons.append(f"🛡️ حماية مفعلة: آمن من فخاخ التصريف، والمسافة عن EMA 50 (4H) طبيعية ({distance_from_ema50_4h * 100:.1f}%).")                               
            # ==========================================
            # 🕳️ [ الاستراتيجية 4: الثقب الأسود الشرائي (The Black Hole Absorption) ]
            # ==========================================
            # استخراج المتغيرات الخاصة بالاستراتيجية
            fvg_gap_size = float(coin.get('fvg_gap_size') or 0.0)
            trend_1h = coin.get('1h_trend_direction', 'عرضي')
            
            # 1. القطيع يبيع بضراوة ويظن أن السوق هابط
            is_retail_selling = (taker_buy_ratio_1h < 1.0)
            
            # 2. انعدام الفجوات (السعر محشور ويتم تجميع السيولة بصمت)
            is_no_fvg = (fvg_gap_size == 0.0)
            
            # 3. التخدير البصري (مسار عرضي يطرد المتداولين الكلاسيكيين)
            is_sideways_1h = (trend_1h == 'عرضي')
            
            # 4. بركان داخلي يغلي (زخم قوي مخفي داخل المسار العرضي)
            is_high_adx = (adx_1h > 40)
            
            # 5. الماكد في منطقة السلبية الكاذبة (قريب جداً من الصفر أو سلبي طفيف)
            # نستخدم نسبة مئوية من السعر لضمان دقة الحسبة مع اختلاف أسعار العملات
            is_fake_negative_macd = (macd_hist_1h <= 0.0) and (abs(macd_hist_1h) < (price * 0.005))
            
            # 6. السعر ملامس أو قريب جداً من المتوسطات (تجميع محكم)
            is_hugging_emas = (abs(price - ema20_1h) / ema20_1h <= 0.01) and (abs(price - ema50_1h) / ema50_1h <= 0.015)

            # 🚀 إطلاق الاستراتيجية
            if is_retail_selling and is_no_fvg and is_sideways_1h and is_high_adx and is_fake_negative_macd and is_hugging_emas:
                score += 350  # تقييم استثنائي لاصطياد الانفجار من القاع المخفي
                signal_type = "LONG"
                
                # 💎 [ الشرط الذهبي ]: تحديد نقاط الارتكاز (Anchor Points) لحساب الترند أو الوقف
                # ضمان تباعد نقاط الارتكاز زمنياً وسعرياً لتجنب ضوضاء السوق (Market Noise) والاختراقات الكاذبة
                # نتجاهل قيعان فريم الساعة المتقاربة، ونبحث عن ارتكازات واسعة بين فريم 4H و اليومي
                valid_anchor_distance = abs(sup_4h - sup_1d) / sup_1d > 0.02 
                black_hole_sl = sup_4h if valid_anchor_distance else (price * 0.95)
                
                reasons.append("🕳️ الثقب الأسود الشرائي: الحيتان تمتص بيوع القطيع بأوامر معلقة (Taker < 1) داخل مسار عرضي مخدر.")
                reasons.append(f"🌋 غليان خفي: المؤشرات هادئة والماكد سلبي كاذب، لكن الزخم الداخلي ينفجر (ADX: {adx_1h:.1f}) مع انعدام الفجوات (FVG=0).")
                reasons.append(f"🛡️ تم تأمين الصفقة بوقف خسارة ({black_hole_sl:.4f}) معتمد على نقاط ارتكاز متباعدة وغير متجاورة لتجنب ذيول صانع السوق.")
            ...
            for tf, pattern in patterns.items():
                ...
                if tf == '1d':
                    ...
                    if has_volume_confirmation: 
                        if is_bullish and (is_at_tf_support or is_uptrend):
                            score += weight
                            reasons.append(f"🟢 1D {clean_name} ")
                ...
                # فريم 4H
                    ...
                    if has_volume_confirmation:
                        if is_bullish and is_at_tf_support:
                            score += weight
                            reasons.append(f"🟢 4H {clean_name} ")
                ...
                # فريم 1H
                    ...
                    if has_volume_confirmation:
                        if is_bullish and is_at_tf_support:
                            score += weight
                            reasons.append(f"🟢 1H  {clean_name} ")
                        elif is_bearish and is_at_tf_resistance:
                            score -= int(weight * 1.5)
                            reasons.append(f"🔴 1H {clean_name} ")
                ...
                # فريم 15m
                    ...
                    if is_bullish and rsi_15m <= 35 and is_uptrend:
                        score += weight
                        reasons.append(f"🟢 15m {clean_name} ")
                ...
                # فريم 5m
                    ...
                    if is_sqz:
                        ...
                        reasons.append(f"🤔 5m {clean_name}")

            # ==========================================
            # 🌪️ [ 6. المحرك الشامل الماكرو/مايكرو والغطاء الجوي ]
            # ==========================================
            ...
            # ---------------------------------------------------------
            # 2. 🚀 زحف الإعصار (LONG) / 🩸 الانهيار الجليدي (SHORT)
            # ---------------------------------------------------------
            # 1h
            is_crawling_up_1h = (price >= ema20_1h) and (price >= bb_upper_1h * 0.995) and (ema20_1h > ema50_1h > ema100_1h)
            is_crawling_down_1h = (price <= ema20_1h) and (price <= coin.get('bb_lower_1h', 0) * 1.005) and (ema20_1h < ema50_1h < ema100_1h)
            
            if is_crawling_up_1h: score += weight_macro; reasons.append(f"🚀 زحف إعصار مؤكد 1h")
            elif is_crawling_down_1h: score -= weight_macro; reasons.append(f"🩸 انهيار جليدي مؤكد 1h")

            # 4h - الحقيقة المطلقة
            is_crawling_up_4h = (price >= ema20_4h) and (ema20_4h > ema50_4h) and (rsi_4h > 55)
            is_crawling_down_4h = (price <= ema20_4h) and (ema20_4h < ema50_4h) and (rsi_4h < 45)
            
            if is_crawling_up_4h: score += weight_god; reasons.append(f"🚀🌪️ هيمنة مشترين مطلقة 4h")
            elif is_crawling_down_4h: score -= weight_god; reasons.append(f"🩸🌪️ هيمنة بائعين مطلقة 4h")

            # 1h
            kc_up_1h = float(coin.get('kc_upper_1h') or 99999)
            kc_dn_1h = float(coin.get('kc_lower_1h') or 0)
            if (bb_upper_1h > kc_up_1h) and expansion_ratio_1h > 1.05:
                score += weight_macro; reasons.append("🌋 كسر انضغاط للأعلى 1h")
            elif (coin.get('bb_lower_1h', 0) < kc_dn_1h) and expansion_ratio_1h > 1.05:
                score -= weight_macro; reasons.append("🕳️ كسر انضغاط للأسفل 1h")

            # ---------------------------------------------------------
            # 3. 📊 الفوليوم المضاعف (Volume Spikes)
            # ---------------------------------------------------------
            vol_1h = float(coin.get('volume_1h') or 0)
            vol_ma_1h = float(coin.get('volume_ma_1h') or 1)
            vol_4h = float(coin.get('volume_4h') or 0)
            ...
            # ---------------------------------------------------------
            # 4. 🌪️ قوة الاتجاه (ADX) كمعزز للاتجاه الحالي
            # ---------------------------------------------------------
            adx_1h = float(coin.get('adx_1h') or 0)
            adx_4h = float(coin.get('adx_4h') or 0)
            ...
            if adx_4h > 25:
                if is_crawling_up_4h: score += 40; reasons.append(f"🔥 مسار ماكرو شرائي لا يُقهر (ADX 4H)")
                elif is_crawling_down_4h: score -= 40; reasons.append(f"🩸 مسار ماكرو بيعي مدمر (ADX 4H)")
            ...
            for tf, pat_name in tf_patterns.items():
                ...
                # ----------------------------------------------------
                    ...
                    if 50 <= pattern_apex_progress <= 75 and is_body_close == 1:
                        score += weight
                        reasons.append(f"📐ا {tf_label} اختراق {pat_name}")
                    elif pattern_apex_progress > 75:
                        reasons.append(f"⚠️ا {tf_label} ضعف {pat_name}")
                        
                elif pat_name in bearish_triangles:
                    if 50 <= pattern_apex_progress <= 75 and is_body_close == 1:
                        score -= int(weight * 1.5)
                        reasons.append(f"📉ا {tf_label} كسر {pat_name}")
                ...
                # ----------------------------------------------------
                    ...
                    if divergence_4h == "BULLISH_DIVERGENCE" and tf in ['4H', '1D']:
                        score += int(weight * 1.5)
                        reasons.append(f"🚀ا {tf_label} انعكاس قوي {pat_name}")
                    else:
                        score += weight
                        reasons.append(f"⚡ا {tf_label} اختراق {pat_name}")
                ...
                # ----------------------------------------------------
                    ...
                    if divergence_4h == "BULLISH_DIVERGENCE":
                        ...
                        score += weight
                        reasons.append(f"📈ا {tf_label} نموذج شرائي {pat_name}")
...
async def private_start_handler(message: types.Message):
    ...
    welcome_msg = (
        ...
        f"• تنبيه يومي للعملات  للارتفاع أو الهبوط مع تحديد النسب بدقة متناهية.\n"
        ...
        f"▫️ 3 أشهر: <b>4500$</b>\n"
        f"▫️ 6 أشهر: <b>6500$</b>\n"
        f"▫️ سنة كاملة: <b>1,0000$</b>\n\n"
        f"<i>🤍 ملاحظة: جميع أموال الاشتراكات تذهب لدعم الفقراء واليتامى ابتغاء وجه الله تعالى اما انا مكتفي بما تلمني ربي واعطاني من فضله.</i>\n\n"
