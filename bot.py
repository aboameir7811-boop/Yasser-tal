...
def validate_strict_trendline(df, x1, y1, x2, y2, trend_type="UP"):
    """
    تطبيق القاعدة: مسموح يقطع الذيل، لكن ممنوع يقطع جسم الشمعة
    """
    slope = (y2 - y1) / (x2 - x1)
...
def calculate_price_channel(df, best_trend, swings_high, swings_low):
    """
    بناء القناة السعرية واكتشاف (شمعة الكسر) و (شمعة الاختبار) كما في الكورس.
    """
    channel_data = {
        "channel_upper": 0.0,
        "channel_lower": 0.0,
    ...
    prev_index = len(df) - 2 # الشمعة السابقة لمعرفة الكسر والاختبار
    ...
    avg_price = df['close'].mean()
    tolerance = avg_price * 0.001 # نسبة التسامح لحساب اللمسات
    ...
    if best_trend["direction"] == "UP":
        ...
        # حساب اللمسات على الخط العلوي
        parallel_touches = 0
        for px, py in swings_high:
            expected_y = (slope * px) + max_intercept
            if abs(py - expected_y) <= tolerance:
                parallel_touches += 1
                
        total_touches += parallel_touches
        ...
        # حساب اللمسات على الخط السفلي
        parallel_touches = 0
        for px, py in swings_low:
            expected_y = (slope * px) + min_intercept
            if abs(py - expected_y) <= tolerance:
                parallel_touches += 1
                
        total_touches += parallel_touches
        channel_data["channel_lower"] = round((slope * current_index) + min_intercept, 4)
        prev_lower = (slope * prev_index) + min_intercept
        prev_upper = (slope * prev_index) + best_trend["current_line_price"] - (slope * current_index)
        
    channel_data["channel_touches"] = total_touches
    
    # === اكتشاف حالة السعر (كسر / اختبار / داخل القناة) بالملّي ===
    
    current_upper = channel_data["channel_upper"]
    current_lower = channel_data["channel_lower"]
                    ...
                    if results[i] and isinstance(results[i], list):
                        ...
                        if tf in ['1w', '1M']:
                            record.update({
                                ...
                                f"{tf}_channel_touches": channel_info["channel_touches"],
